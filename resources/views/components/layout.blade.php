	...
	<body>
								...
								<li class="nav-item text-center">
									<div class="dropdown text-end my-2">
										<a href="#" class="d-block link-dark text-decoration-none dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
											<img src="{{ cloudinary()->getUrl('images/' . auth()->user()->image) }}" alt="{{ auth()->user()->name }}" width="32" height="32" class="rounded-circle">
										</a>
		...
		@if (session('message'))
			<div x-data="{show: true}" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition.duration.500ms>				
				<div class="alert alert-{{ session('status') }} mb-5 rounded-0" role="alert" id="alert-box">
					{{session('message')}}
				</div>
			</div>
		@endif
